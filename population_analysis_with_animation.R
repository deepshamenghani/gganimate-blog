...
population_dataset <- readxl::read_xlsx("00_data/population_and_density_change_data_table_US_by_state_1910to2020.xlsx") %>% 
...
state_list <- data %>% distinct(Name) %>% pull()
...
plot_density_vs_rank <- function(data, state_input) {
    ...
    plot <- data_filtered %>%
        ...
        geom_point(aes(text = label_plotly), alpha = 0.5,
                   show.legend = FALSE, shape=shape_input, size = 4) +
